...
export async function checkStaleCronPayloads(
  ...
  // Only workflow-agent crons (antfarm/<workflow>/<agent>) carry buildPollingPrompt() output.
  // antfarm/medic is generated by buildMedicPrompt() in ./medic-cron.ts and can never contain
  // the polling markers, so including it raised a false "stale payload" warning on every single
  // scheduled run — masking real findings and preventing a clean HEARTBEAT_OK.
  // Confirmed 2026-08-30 (TASK-031); antfarm-doctor.mjs Check 1 already carries this exclusion.
  const antfarmCrons = cronJobs.filter((j) => /^antfarm\/[^/]+\/[^/]+$/.test(j.name));
  const workflowIds = new Set<string>();
